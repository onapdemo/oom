...
function usage
{
	...
	echo " "
	echo "       demo.sh instantiateAzureVFW"
    echo "               - Instantiate AzurevFW module for the a demo customer (DemoCust<uuid>)"
	echo " "
	echo "       demo.sh instantiateAzureVDNS"
    echo "               - Instantiate AzurevDNS module for the a demo customer (DemoCust<uuid>)"
	echo " "
...
do
	...
	case $key in
    	...
    	instantiateVFW)
			...
			;;
    	instantiateAzureVFW)
			TAG="instantiateAzureVFW"
			VARIABLES="$VARIABLES -v GLOBAL_BUILD_NUMBER:$$"
			shift
			;;
    	instantiateAzureVDNS)
			TAG="instantiateAzureVDNS"
			VARIABLES="$VARIABLES -v GLOBAL_BUILD_NUMBER:$$"
			shift
			;;
    	deleteVNF)
